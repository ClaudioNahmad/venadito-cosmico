...
/** funcion que facilita la expresion de w_fld */
int a_function(
           struct background *pba,
           double a,
           double * a_trans,
...
int background_w_fld(
                     struct background * pba,
                     double a,
                     double * q_factor,			/*TODO quitar q_factor de aqui y definirlo localmente abajo o revisar si se puede definir dentro de los ()'s como acuí*/
                     double * w_fld,
  ...
  /** *w_fld = pba->w0_fld + pba->wa_fld * (1. - a / pba->a_today);  ECUACION ORIGINAL */ 
  *w_fld = pba->w0_fld + pba->wa_fld * ((pow(pba->a_func, pba->q_factor)) / (1. + pow(pba->a_func, pba->q_factor)));
/**************************************************************************************************************************/


/** HIKURI 12-abr-18
	AQUI VA EL CONDICIONAL:
		Si a>a_0 => w_fld=w0
		si no, w_fld es igual a lo que puse arriba

	ESTE CONDICIONAL ES PARA EVITAR DIVERGENCIA (PREGUNTARLE A MARIANA) 
	*/
...
/**************************************************************************************************************************/
  ...
  /** *dw_over_da_fld = - pba->wa_fld / pba->a_today; ECUACION ORIGINAL */
  *dw_over_da_fld =  - pba->wa_fld * (pow(
          ((pba->a_trans * pba->q_factor) / ((1. - pba->a_trans) * pow(a, 2))) * pba->a_func, pba->q_factor - 1.) /
                                      (pow(1. + pow(pba->a_func, pba->q_factor), 2))  );
  ...
  /** *integral_fld = 3.*((1.+pba->w0_fld+pba->wa_fld)*log(pba->a_today/a) + pba->wa_fld*(a/pba->a_today-1.)); ECUACION ORIGINAL */
  *integral_fld = 0;
